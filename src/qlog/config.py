...
class Config:
    def __init__(self):
        ...
        try:
            ...
            self.ISSUE_URL  = key("issue_url","project","issue",d="https://github.com/{project}/issues/{issue}") # project, issue
            self.PR_URL     = key("pr_url"   ,"project","pr"   ,d="https://github.com/{project}/pull/{pr}") # project, pr
            ...
            self.ENTRY_FILENAME_FORMAT = key("entry_filename_format","time","title",d="{time}_{title}.md") # time, title

            self.DATE_FORMAT = key("date_format",d="%d %b, %Y") # use standard strftime parameters
            self.TITLE_FORMAT = key("title_format","version","date",d="## {version} - *{date}*") # version, date
